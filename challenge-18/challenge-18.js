...
cpfs.forEach(function(value, key){
    console.log(cleanCPF(value));
});
/*
...
cpfs.forEach(function(value, key){
    console.log(cleanCPF(value).replace(/(\d{3})(\d{3})(\d{3})(\d{2})/,'$1.$2.$3-$4'));
});
...
var marcation = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'
var regex = /<(\w+)>([^<]+)<\/\w+>/g;

function replaceText( regex, tag, text ) {
    return "\n<" + tag + "> o texto dentro da tag " + tag + " é " + text + " <" + tag + ">";
}
console.log( marcation.replace(regex, replaceText) );
